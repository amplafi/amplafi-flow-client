...
public class AdminTool extends UtilParent {
	...
	private static final Pattern scriptPattern = Pattern
			.compile("^(.*)\\.groovy$");
	...
	public AdminTool() {
		...
		try {
			this.props
					.load(new FileInputStream("fareaches.fadmin.properties"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	 ...
	 * @param cmdOptions
	 *            is instance of AdminToolCommandLineOptions
	 */
	private void runScript(String filePath) {
		...
		String apiVersion = props.getProperty(API_VERSION); // ,
															// DEFAULT_API_VERSION);
		FarReachesServiceInfo serviceInfo = new FarReachesServiceInfo(host,
				port, apiVersion);
		...
		ScriptRunner runner = new ScriptRunner(serviceInfo); // , apiKey,
