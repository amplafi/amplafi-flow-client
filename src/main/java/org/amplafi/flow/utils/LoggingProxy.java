 ...
 * http://www.java2s.com/Code/Java/Network-Protocol/Asimpleproxyserver.htm Runs
 * a proxy between the plugin and the wire-server. Detects requests and resposes
 * and out puts a single integration style test in the test DSL format.
 *
 * @author paul
 */
public class LoggingProxy {
	...
	protected LoggingProxyCommandLineOptions cmdOptions;
	private Log log;
	...
	/**
	 * Using a jetty server will probably have many advantages over the code
	 * below especially in terms of reducing complexity to of code in threading
	 * and processing the http request.
	 *
	 * @param host
	 *            is host that run proxy
	 * @param remotePort
	 *            is remote port
	 * @param localPort
	 *            is local port
	...
	public void runJettyProxy(String host, int remotePort, int localPort) {
		 ...
		 * SelectChannelConnector(); connector.setPort(8888);
		 * server.setConnectors(new Connector[]{connector}); ServletHandler
		 * handler=new ServletHandler(); server.setHandler(handler);
		 * //FilterHolder gzip =
		 ...
		 * handler
		 * .addServletWithMapping("org.eclipse.jetty.servlets.ProxyServlet"
		 * ,"/"); proxy.setAsyncSupported(true); server.start(); server.join();
		 */
	}
	 ...
	 *             IOException
	 */
	protected Writer getTestFileWriter() throws Exception {
	...
	/**
	 * This method can get the flow name and params from the requestUrl and save
	 * them into a map.
	 *
	 * @param req
	 *            the request we get from the proxy server.
		...
		if (req != null) {
			...
			if (tokens.length > 4) {
				...
				if (flowCallTokens.length == 2) {
					...
					for (String element : paramsString) {
						String[] paramMap = element.split("=");
	 ...
	 *
	 * @param strValue
	 *            is the String write to FileBuffer
	 */
