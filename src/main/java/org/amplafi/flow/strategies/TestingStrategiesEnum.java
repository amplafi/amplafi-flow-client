...
/**
 * Just a simple enum for managing the available strategies. Nothing more
 * complicated is needed just now.
 * 
 * @author paul
 */
public enum TestingStrategiesEnum {

	BogusString(new BogusStringDataStrategy()), CorruptParams(
			new CorruptParamsNameTestingStrategy()), NullParam(
			new NullParamStrategy()), ParamCombinations(
			new CombinationsStrategy()), RealisticParams(
			new RealisticParamsStrategy());
